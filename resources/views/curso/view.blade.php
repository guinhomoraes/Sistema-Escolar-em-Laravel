<x-layout>

    <div class="container-fluid">
        <div class="row">

            <div class="col-12 d-flex justify-content-between 
                        align-items-end border-bottom border-secondary py-3">

                <h3 class="my-0"> <span class="font-weight-bold"> Curso </span> - Visualizar Curso</h3>
                <a class="btn btn-primary" href="{{ route('curso.index') }}">Listar Cursos</a>

            </div>

            <div class="col-5">

                <div class="row">

                    <div class="col-12 col-md-8 col-lg-8 col-xl-8 col-xxl-8 my-2">

                        <div class="form-group m-0">
                            <label for="">Nome</label>
                            <input id="nome" class="form-control" type="text" name="nome" 
                              value="{{ $modelCurso->nome }}" readonly>
                        </div>
                    
                    </div>

                    <div class="col-12 col-lg-4 col-xl-4 col-xxl-4 my-2">

                        <div class="form-group m-0">
                            <label for="">Status</label>
                            <input id="status" class="form-control" type="text" name="status" 
                            value="{{ $modelCurso->status == 1 ? "Ativo" : "Inativo" }}" readonly>
                        </div>

                    </div>

                    <div class="col-12 col-md-8 col-lg-8 col-xl-8 col-xxl-8 my-2">

                        <div class="form-group m-0">
                            <label for="">Descrição</label>
                            <input id="descricao" class="form-control" type="text" name="descricao" 
                            value="{{ $modelCurso->descricao }}" readonly>
                        </div>

                    </div>

                    <div class="col-12 col-lg-4 col-xl-4 col-xxl-4 my-2">

                        <div class="form-group m-0">
                            <label for="">Valor</label>
                            <input id="preco" class="form-control" type="text" name="preco" 
                            value="{{ $modelCurso->preco }}" readonly>
                        </div>

                    </div>

                    <div class="col-12 my-2">

                        <label for="">Observação</label>

                        <textarea name="observacao" id="observacao" class="form-control" cols="30" rows="5" readonly>{{ $modelCurso->observacao }}</textarea>

                    </div>

                </div>

            </div>

        </div>
    </div>

</x-layout>